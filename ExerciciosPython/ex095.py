...
def quantos_gols(a):
    while True:
        qg = int(input('Digite o número de gols que o jogador fez na partida {}: '.format(a+1)))
        if qg < 0 or qg > 13:
            print('\033[1;31mERRO! Entrada inválida! Tente novamente!\033[m')
        else:
            return qg
...
while True:
    ...
    if i > 0:
        ...
        dic['qtd partidas'] = i
        i = 0
        dic['aproveitamento'] = dic['total de gols'] / dic['qtd partidas']
    else:
        dic['qtd partidas'] = 0
...
for j in range(0, len(players)):
    if players[j]['qtd partidas'] == 0:
        ...
        print(f'|{"Total de gols":<63}|{0:^24}|')
        print(f'|{"Aproveitamento (gols por partida)":<63}|{0:^24}|')
        print('=' * 90)
    else:
        for k in range(0, players[j]['qtd partidas']):
            if k == 0:
                ...
                if k == (players[j]['qtd partidas'] - 1):
                    ...
                    print(f'|{"Aproveitamento (gols por partida)":<63}|{round(players[j]["aproveitamento"], 2):^24}|')
                    print('=' * 90)
print('=' * 90)
